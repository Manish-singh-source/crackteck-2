...
class PayToVendorController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('/crm/accounts/payments-to-vendors/index', compact('vendorPurchaseBills'));
    }
    ...
    public function view($id)
    {
        ...
        return view('/crm/accounts/payments-to-vendors/view', compact('vendorPurchaseBill'));
    }
    ...
    public function edit($id)
    {
        ...
        if (! $vendorPurchaseBill) {
            return redirect()->route('pay-to-vendors.index')->with('error', 'Record not found.');
        }

        return view('/crm/accounts/payments-to-vendors/edit', compact('vendorPurchaseBill', 'vendors'));
    }
    ...
    public function destroy($id)
    {
        ...
        if ($vendorPurchaseBill->products_count > 0) {
            return redirect()->route('pay-to-vendors.index')->with('error', 'Cannot delete vendor purchase bill with products.');
        }

        $vendorPurchaseBill->delete();

        if (! $vendorPurchaseBill) {
            return redirect()->route('pay-to-vendors.index')->with('error', 'Failed to delete vendor purchase bill. Please try again.');
        }

        return redirect()->route('pay-to-vendors.index')->with('success', 'Vendor Purchase Bill deleted successfully.');
    }
}
