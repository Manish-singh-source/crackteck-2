        ...
        <div class="container-fluid">
            <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
                <div class="flex-grow-1">
                    <h4 class="fs-18 fw-semibold m-0">Device Specific Diagnosis List</h4>
                </div>
                <div>
                    <a href="{{ route('device-specific-diagnosis.create') }}" class="btn btn-primary">Add Device Specific Diagnosis</a>
                </div>
            </div>
                                ...
                                @foreach ($statuses as $key => $status)
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link {{ $currentStatus === $key ? 'active' : '' }} p-2"
                                            href="{{ $key === 'all' ? route('device-specific-diagnosis.index') : route('device-specific-diagnosis.index', ['status' => $key]) }}">
                                                    ...
                                                    <table id="responsive-datatable"
                                                        ...
                                                        <tbody>
                                                            @forelse ($diagnosis as $item)
                                                                <tr>
                                                                    <td>{{ $item->id }}</td>
                                                                    <td>{{ ucwords(str_replace('_', ' ', $item->device_type)) }}</td>
                                                                    ...
                                                                    <td>
                                                                        <a aria-label="anchor"
                                                                            href="{{ route('device-specific-diagnosis.edit', $item->id) }}"
                                                                        ...
                                                                        <form
                                                                            action="{{ route('device-specific-diagnosis.delete', $item->id) }}"
                                                                            method="POST" style="display: inline-block;"
                                                                            onsubmit="return confirm('Are you sure you want to delete this device specific diagnosis?');">
                                                                            @csrf
                                                                            @method('DELETE')
